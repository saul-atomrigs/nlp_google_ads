...
def news_text(opener, news):
    ...
    soup = bs(requests.get(url, headers={
              'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_13_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.164 Safari/537.36'}).text, "html.parser")

    articleBodyContents = soup.select("#articleBodyContents")
    if len(articleBodyContents) == 0:
        news["text"] = ""
    else:
        news["text"] = html_to_text(articleBodyContents[0])
    return news
...
def news_list_item(soup):
    dataset = []
    items = soup.select(
        "#main_content > div.list_body.newsflash_body > ul > li > dl > dt[class!='photo'] > a")
    for item in items:
...
def thread_runner(t_id, args, output, news_set):
    ...
    while zeros < 5 and 0 < DATE_QUEUE.qsize():
        ...
        try:
            ...
            count = crawel_news_date(
                t_id, args, output, news_set, opener, date)
            if count is not None:
                ...
                logger.info(
                    f"Thread: {t_id:2d} / Date: {date} / Count: {count:4d} / Time: {duration.total_seconds():6.2f} / Remain: {DATE_QUEUE.qsize():5d}")
        except Exception as ex:
            print(traceback.format_exc())
            logger.info(
                f"Thread: {t_id:2d} / Date: {date} / Exception: {ex} / Remain: {DATE_QUEUE.qsize():5d}")
...
def crawel_news(args):
    ...
    max_date = datetime.datetime.now().replace(
    ...
    for t_id in range(args.threads):
        thread = threading.Thread(
            target=thread_runner, args=(t_id, args, output, news_set))
        thread.start()
        time.sleep(0.1)
...
if __name__ == "__main__":
    ...
    log_handler = handlers.TimedRotatingFileHandler(
        filename="log/naver_news_csv.log", when="midnight", interval=1, encoding="utf-8")
